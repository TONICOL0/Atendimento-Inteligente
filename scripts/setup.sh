#!/bin/bash
# ============================================
# Konecta - Script de Setup Inicial
# ============================================
...
echo "============================================"
echo "  Konecta Atendimento Inteligente - Setup"
echo "============================================"
...
until docker exec konecta-postgres pg_isready -U konecta > /dev/null 2>&1; do
...
until docker exec konecta-redis redis-cli -a "$(grep REDIS_PASSWORD .env | cut -d= -f2)" ping > /dev/null 2>&1; do
...
docker exec konecta-postgres psql -U konecta -c "SELECT 1 FROM pg_database WHERE datname='chatwoot_production'" | grep -q 1 || \
  docker exec konecta-postgres psql -U konecta -c "CREATE DATABASE chatwoot_production;"
docker exec konecta-postgres psql -U konecta -c "SELECT 1 FROM pg_database WHERE datname='typebot'" | grep -q 1 || \
  docker exec konecta-postgres psql -U konecta -c "CREATE DATABASE typebot;"
docker exec konecta-postgres psql -U konecta -c "SELECT 1 FROM pg_database WHERE datname='evolution_v2'" | grep -q 1 || \
  docker exec konecta-postgres psql -U konecta -c "CREATE DATABASE evolution_v2;"
...
docker exec konecta-chatwoot bundle exec rails db:chatwoot_prepare
...
echo ""
echo "  docker exec -it konecta-chatwoot bundle exec rails console"
echo ""
echo "  No console Rails, execute:"
echo "  SuperAdmin.create!(email: 'admin@konecta.adv.br', password: 'SuaSenhaForte123!', name: 'Admin Konecta')"
echo ""
...
echo "Criando instância: konecta-principal..."
curl -s -X POST "${EVOLUTION_URL}/instance/create" \
  -H "apikey: ${API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "instanceName": "konecta-principal",
...
echo "Criando instância: konecta-secundario..."
curl -s -X POST "${EVOLUTION_URL}/instance/create" \
  -H "apikey: ${API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "instanceName": "konecta-secundario",
    "integration": "WHATSAPP-BAILEYS",
    "qrcode": true
  }' | head -c 200
echo ""
...
# Instância 3: Teste
echo "Criando instância: konecta-teste..."
curl -s -X POST "${EVOLUTION_URL}/instance/create" \
  -H "apikey: ${API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "instanceName": "konecta-teste",
    "integration": "WHATSAPP-BAILEYS",
    "qrcode": true
  }' | head -c 200
echo ""
...
echo "   curl http://localhost:8081/instance/connect/konecta-principal -H 'apikey: ${API_KEY}'"
echo "   curl http://localhost:8081/instance/connect/konecta-secundario -H 'apikey: ${API_KEY}'"
echo "   curl http://localhost:8081/instance/connect/konecta-teste -H 'apikey: ${API_KEY}'"
echo ""
echo "============================================"
